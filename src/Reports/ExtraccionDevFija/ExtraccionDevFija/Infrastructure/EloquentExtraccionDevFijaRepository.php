...
class EloquentExtraccionDevFijaRepository implements ExtraccionDevFijaRepository
{
    ...
    public function processAndGetGruposUsuario($distritos, $ticket, $servicioAfectado, DateTime $fechaIni, DateTime $fechaFin, $mesesInteres)
    {
        ...
        ->statement("BEGIN
            DELETE FROM USRAES.DWH_DEVOLUCION_MASIV_DETALLE_{$this->userIdentifier}
            WHERE FUENTE='SGA' AND to_number(CICFAC_DEVOL) in (
        ...
        $queries[] = ["sql" => "BEGIN
            ...
            CR_NETOCIGV,
            MONTO_PRINCIPAL,
            CASE WHEN MOVIL_STATUS='S' THEN AGREEMENT_STATUS_DATE WHEN MOVIL_STATUS IN ('A','G') THEN NULL WHEN (MOVIL_STATUS NOT IN ('A','G') AND AGREEMENT_END_DATE<FCHFIN_INST) OR TRUNC(AGREEMENT_END_DATE)=TO_DATE('31/12/9999','DD/MM/YYYY') OR AGREEMENT_END_DATE IS NULL THEN FCHFIN_INST ELSE AGREEMENT_END_DATE END FECHA_BAJA,
		...
		SBS.AGREEMENT_STATUS_DATE,
            ...
            ON SBS.ID_CARD_VALUE=TK.NRO_DOC AND SUBSCRIPTION_SOURCE_SYSTEM_DESC IN ('BSCS','BSCSIX')-- AND SBS.AGREEMENT_SERVICE_GROUP='MOVIL'
        ...
        $queries[] = ["sql" => "BEGIN
            ...
            WHERE ((CASE FUENTE
            WHEN 'BSCS' THEN (CASE WHEN ESTADO_CONTRATO NOT IN ('D','S') THEN 1 ELSE 0 END)
            WHEN 'SGA' THEN (CASE WHEN CICFAC_DEVOL IS NOT NULL AND FCHFIN_INST IS NULL THEN 1 ELSE 0 END)
            ELSE 0 END
            ) = 0
            ...
            WHEN 'BSCS' THEN (CASE WHEN DMD.ESTADO_CONTRATO NOT IN ('D','S') THEN 1 ELSE 0 END)
