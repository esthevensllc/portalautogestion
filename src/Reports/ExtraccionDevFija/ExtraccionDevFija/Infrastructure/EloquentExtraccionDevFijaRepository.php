...
class EloquentExtraccionDevFijaRepository implements ExtraccionDevFijaRepository
{
    ...
    public function getReportePostpago($ticket, $fuente)
    {
        ...
        ROUND(CASE
            WHEN MINUTOS_AFECTACION >= 1440 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 4
            WHEN MINUTOS_AFECTACION >= 700 AND MINUTOS_AFECTACION < 1440 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 3.5
            WHEN MINUTOS_AFECTACION >= 300 AND MINUTOS_AFECTACION < 700 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 3
            WHEN MINUTOS_AFECTACION >= 60 AND MINUTOS_AFECTACION < 300 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 2.5
            ELSE ROUND(MONTO_PRINCIPAL * 1.18, 2)
        END, 2) AS COMPENSACION,
        ROUND(CASE
            WHEN MINUTOS_AFECTACION >= 1440 THEN MINUTOS_AFECTACION * 4
            WHEN MINUTOS_AFECTACION >= 700 AND MINUTOS_AFECTACION < 1440 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 3.5
            WHEN MINUTOS_AFECTACION >= 300 AND MINUTOS_AFECTACION < 700 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 3
            WHEN MINUTOS_AFECTACION >= 60 AND MINUTOS_AFECTACION < 300 THEN ROUND(MONTO_PRINCIPAL * 1.18, 2) * 2.5
            ELSE ROUND(MONTO_PRINCIPAL * 1.18, 2)
        END, 2) / (ROUND(MONTO_PRINCIPAL * 1.18, 2) + INTERES) AS FACTOR_MULTIPLICATIVO,
        INTERES,
        TASA,
