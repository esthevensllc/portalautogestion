    ...
    public function __invoke($cliente, $periodo, $tipo_input, $fecha1, $fecha2)
    {
        ...
        if(($periodo === null && $tipo_input === '1') || (($fecha1 === null || $fecha2 === null) && $tipo_input === '2')){
            ...
            if(count($invalid_periods) > 0){
                ...
                $errors['message'] = "No hay registros para el cliente {$cliente} en los periodos {$str_periodos} en la tabla TEMP_TAG_11 de la DBTODB. Por favor comunicarse con el área de Factory Soporte Facturación - factsopfacturacion@claro.com.pe - COLOCAR EN EL ASUNTO LA PALABRA CLAVE \"TEMP_TAG_11 \"";
            }else{
                $invalid_periods = [];
                foreach($periodos as $p){
