...
class EloquentExtraccionRepository implements ExtraccionRepository
{
    ...
    public function getReporteMontoDevolver(Datetime $fechaInteres, Datetime $corteFechaIni)
    {
        ...
        WHERE EXISTS (
        SELECT 1 FROM USRAES.USER_BASE_BFIN_{$this->userIdentifier} A WHERE A.ESTADO_ACTUAL='D' AND SBS.ID_CARD_VALUE=A.NRO_DOCUMENTO)
        AND SBS.AGREEMENT_STATUS IN ('A','S','G')
        ...
        SELECT S1.*,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.MSISDN_ACTUAL     END MSISDN_DEVOLVER,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.CUSTOMER_ID       END CUSTOMER_ID_DEVOLVER,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.CUSTCODE          END CUSTCODE_DEVOLVER,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.CONTRATO          END CO_ID_DEVOLVER,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.MODO_CONTRATACION END MODO_CONTRATACION_DEV,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.TIPO_CLIENTE      END TIPO_CLIENTE_DEV,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.FECHA_ACTIVACION  END FECHA_ACTIVACION_DEV,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.CICLOFACTURACION  END CICLOFACTURACION_DEV,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN S1.SISTEMA_ORIGEN    END SISTEMA_ORIGEN_DEV,
               CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN 1 ELSE 0             END DEV,
               CAST(CASE WHEN S1.ESTADO_ACTUAL IN ('A','S','G') THEN DECODE(S1.MODO_CONTRATACION,'PREPAGO','PREPAGO',DECODE(S1.TIPO_CLIENTE,'CONSUMER','POSTPAGO_CONSUMER','POSTPAGO_CORPORATIVO')) END AS VARCHAR2(100))   MODALIDAD_DEV,
    ...
    public function getPostpagoBy($ticket, $departamento)
    {
        ...
        ROUND(ROUND(ROUND(MONTO_DEVOLVER_IGV, 2)/1.18, 2) * 1.18, 2) MTO_DEV_IGV,INTERES,TASA,MTO_TOTAL_DEV_IGV MTO_TOTAL_DEV_IGV,CUSTCODE_DEVOLVER AS CUSTCODE, CUSTOMER_ID_DEVOLVER CUSTOMER_ID,CO_ID_DEVOLVER,CICLOFACTURACION_DEV CICLOFACTURACION,SISTEMA_ORIGEN_DEV FUENTE,
