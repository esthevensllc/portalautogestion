...
class MantenimientoCeldaProcessor
{
    private $repo;
    ...
    public function __invoke(?int $step, $tipoInput, $celdas, $provincias, $excel, $fechaIni, $fechaFin, $ticketOsiptel, $fechaInteres, $corteFechaIni, $corteFechaFin, $minutos_usuarios)
    {
        ...
        try {
            ...
            $this->repo->process($arrayCeldas, $arrayProvincias, $dtFechaIni, $dtFechaFin, $ticketOsiptel, $dtFechaInteres, $dtCorteFechaIni, $dtCorteFechaFin);
    ...
    {
        $filename = "MANTENIMIENTO_CELDAS_".$ini->format('YmdHis').".xlsx";
        $data = array_merge([
            'name' => 'MANTENIMIENTO_CELDAS',
            'ini' => $ini->format('Y-m-d H:i:s'),
            'fin' => $fin->format('Y-m-d H:i:s'),
            'trac_name' => 'mantenimiento-celdas',
            "filename" => $allFilename !== null ? $filename : null
        ], $extra_data);
        $this->saveReportLog->__invoke($data, $allFilename, 'MANTENIMIENTO_CELDAS');
    }
}
