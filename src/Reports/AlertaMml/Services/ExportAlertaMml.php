...
class ExportAlertaMml
{
    ...
    public function __construct(AlertaMmlRepository $repo, SaveReportLog $saveReportLog, StorageService $storage)
    {
    ...
    public function __invoke($nintex, $base, $fechaIni, $fechaFin, $whiteList)
    {
        ...
        try {
            ...
            $this->storage->put(SaveReportLog::LOCAL_PATH."/AlertaMml/{$filename}", $content);
            $this->reportLog(SaveReportLog::LOCAL_PATH."/AlertaMml/{$filename}", $filename, $fechaIniExec, new DateTime(), $reporteInput, null);
    ...
    private function reportLog(?string $local_file, ?string $filename, DateTime $ini, DateTime $fin, array $input, ?\Throwable $error)
    {
        ...
        if ($local_file !== null) {
            $this->saveReportLog->sendFileToRemoteServer($local_file, "AlertaMml/{$filename}");
        }
    }
}
