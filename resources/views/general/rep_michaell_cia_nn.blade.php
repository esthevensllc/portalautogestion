    ...
    <div class="card-body">
        <form id="form_export">
            ...
            <div class="mb-3 row">
                ...
                <div class="col-lg-3 col-md-4">
                    <div class="form-group" id="num_cuenta_info">
                        Periodo máximo:<br>
                        Periodo mínimo:<br>
                        Ciclo:
                    </div>
...
$(function() {
    ...
    document.querySelector("input[name=num_cuenta]")
    .addEventListener("change", function(e){
        let num_cuenta = e.target.value;
        fetch(config.info_url+"/"+(num_cuenta!==""?num_cuenta:"-"))
        .then(resp => resp.json())
        .then(resp => {
            document.querySelector("#num_cuenta_info").innerHTML = `Periodo máximo: ${resp.max_period??''}<br>Periodo mínimo: ${resp.min_period??''}<br>Ciclo: ${resp.cycle??''}`;
        });
    });
});
</script>
